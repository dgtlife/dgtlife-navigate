  ...
  _.extend(AppNavigator.prototype, {
    ...
    registerScreen: function (screenName, options) {
      ...
      if (options.pathMask && ! options.generatePath) {
        ...
        throw 'The generatePath() function definition was not provided for this path mask: ' + options.pathMask;
      }
     ...
     * @param {object} screenSpec - an object that contains the 'name', 'isAllowed',
     *                              and 'getScreenDataFromPathParameters' keys
     *                              of the screen object
     * @private
     */
    _getScreenData: function (execResult, screenSpec) {
      "use strict";
      var self = this;
      ...
      screenSpec.getScreenDataFromPathParameters(execResult, function (result, error) {
        if (error) {
          ...
          // Remove the 'getScreenDataFromPathParameters' key from the screenSpec.
          // Add in the screen data. Then check whether the screen is allowed.
          delete(screenSpec['getScreenDataFromPathParameters']);
    ...
    _isScreenAllowed: function (screenSpec) {
      ...
      delete(screenSpec['isAllowed']);
      // Evaluate the screen access control function, or apply the boolean.
      if (_.isFunction(isAllowed)) {
        // It's a function, and it may have various dependencies. We need to
        // wait for the return value to be defined before evaluating the
        // function.
        var __isDefined = function () {
          return ! _.isUndefined(isAllowed());
        };

        self.waitForCondition(__isDefined, function () {
    ...
    toScreen: function (screenName, options) {
      ...
      if (screen) {
        ...
        if (updateBrowserHistory) {
          ...
          pathMask = screen.pathMask || null;
          generatePath = screen.generatePath || null;
          self._updateBrowserHistory(screenName, path, pathMask, generatePath, screenData);
        }
     ...
     * @param {function} generatePath - the (screen-specific) function that
     *                                  generates the path from the path mask
     *                                  and the screen data
     * @param {object} screenData - the screen data associated with a
     *                              parameterized path
     * @private
     */
    _updateBrowserHistory: function (screenName, path, pathMask, generatePath, screenData) {
      "use strict";

      var screenState, params;
      ...
      if (pathMask && params && ! path) {
        ...
        path = generatePath(pathMask, params);
